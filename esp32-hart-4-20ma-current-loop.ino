/**
 * =========================================================================================
 * Project     : ESP32 Industrial 4-20mA Current Loop Transmitter & HART Modem
 * Platform    : ESP32 (Dual-Core Xtensa LX6 / FreeRTOS)
 * Framework   : Arduino IDE 2.0+
 * Author      : Muhammad Fikri
 * License     : MIT
 * Description : Precision analog current loop calibrator and process transmitter with digital HART Bell 202 FSK modulation/demodulation for smart industrial field instrumentation.
 * =========================================================================================
 */
...
void TaskTelemetryBroadcaster(void *pvParameters) {
    ...
    for (;;) {
        if (xSemaphoreTake(g_telemetryMutex, pdMS_TO_TICKS(20)) == pdTRUE) {
            ...
            doc["system"] = "esp32-hart-4-20ma-current-loop";
...
void setup() {
    ...
    Serial.println(F("================================================================="));
    Serial.println(F(" ESP32 Industrial 4-20mA Current Loop Transmitter & HART Modem"));
